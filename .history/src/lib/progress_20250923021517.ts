 ...
 * 逐日解鎖（day1、day2 初始可做；day>=3 要前面全完成）→ 由 UI 依 completedSet 動態計算；
 * 呢度唔額外存「已解鎖日」。
 */
...
/**（可選）提供一個 helper，比 UI 想要就用
 * day1、day2 初始 pending；day>=3 必須前面全完成
 */
export function getDayStateHelper(args: {
  level: Level;
  category: Category;
  ...
  if (!weekUnlocked) return 'locked';
  const t = tokenOf(level, category, week, day);
  if (completedSet.has(t)) return 'done';
  if (day <= 2) return 'pending';
  const allPrev = Array.from({ length: day - 1 }, (_, i) => tokenOf(level, category, week, i + 1))
    .every((x) => completedSet.has(x));
  return allPrev ? 'pending' : 'locked';
}
