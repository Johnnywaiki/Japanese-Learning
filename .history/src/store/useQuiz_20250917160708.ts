...
type State = {
  ...
  pool: PoolQuestion[];
  idx: number;                                // 目前第幾題（0-based）
  current?: ReturnType<typeof makeQuestion>;

  selected?: Option | null;                   // 當前選擇（未提交）
  answered: boolean;                          // 當前題是否已提交
  lastCorrect?: boolean;

  score: number;                              // 已答對題數
  total: number;                              // 已作答題數
  totalAvailable: number;                     // 全卷題數
...
export const useQuiz = create<State>((set, get) => ({
  ...
  pick(o) {
    ...
    if (answers[idx]) return;      // 呢題已有提交紀錄，唔俾再揀
    set({ selected: o });
  },
  ...
  submit() {
    ...
    if (answers[idx]) return; // 之前已提交過，唔好重覆計分
