...
export async function ensureSeedOnce(): Promise<void> {
  ...
  const count = (sql: string) => (db.getFirstSync?.<{ c: number }>(`SELECT COUNT(*) as c FROM ${sql}`)?.c ?? 0);
  const counts = {
    exams: count('exams'),
