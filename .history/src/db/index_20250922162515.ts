...
export function getAllForFilter(f: PracticeFilter): { pool: PoolQuestion[] } {
  ...
  if (effMonth && effMonth !== 'random') {conds.push(`(e.month = ? OR ltrim(e.month,'0') = ?)`);
params.push(effMonth, String(Number(effMonth))); // "07" → "7"
  }

// language = 詞彙/文法；有啲資料會以 'language' 一個大類存，所以要相容埋
  if (f.kind === 'language') conds.push(`q.section IN ('vocab','grammar','language')`);
  else if (f.kind === 'reading') conds.push(`q.section = 'reading'`);
  else conds.push(`1=0`);

  const whereSQL = conds.length ? `WHERE ${conds.join(' AND ')}` : '';

  type QRow = { exam_key: string; question_number: number; section: Section; stem: string; passage: string | null; };
  const qs =
    (db.getAllSync?.<QRow>(
      `
