...
/** 由 Supabase 拉 mock + daily 寫入本地 SQLite（覆蓋式 upsert） */
export async function syncFromSupabaseToDB(db: SQLiteDatabase): Promise<boolean> {
  try {
    ...
    const ds = await sb.from('daily_sets').select('*').limit(10000);
    if (ds.error) throw ds.error;
    const dq = await sb.from('daily_questions').select('*').limit(100000);
    if (dq.error) throw dq.error;
    const dc = await sb.from('daily_choices').select('*').limit(200000);
