...
function pickJP(q: any, passageMap: Map<number, any>): string | null {
  const self =
    q.jp ?? q.ja ?? q.stem ?? q.prompt ?? q.text ?? q.content ?? q.question ?? q.title ?? null;
...
function isCorrectFlag(c: any): boolean {
...
/** 讀 exams（固定欄位；不選 exam_code 以避免型別衝突） */
...
export async function syncFromSupabaseToDB(db: SQLiteDatabase): Promise<boolean> {
  ...
  try {
    ...
    // 統計用
    const stat = {
      links: (EQ.data ?? []).length,
      noExam: 0, noQ: 0, noJP: 0, noChoices: 0, noCorrect: 0, noZh: 0, skippedPaper: 0,
      w: 0, s: 0,
    };
    ...
    try {
      ...
      for (const link of (EQ.data ?? [])) {
        ...
        if (!jp) { stat.noJP++; continue; }

        const chs = (choicesByQ.get(q.id) ?? []);
        if (chs.length === 0) { stat.noChoices++; continue; }

        // 1) 優先用 choices 上的正解旗標
        let correct = chs.find(isCorrectFlag);

        // 2) 如果 choices 冇旗標，試 questions.correct_choice_id / answer_choice_id / correct_id…
        const qCorrectId =
          q.correct_choice_id ?? q.answer_choice_id ?? q.correct_id ?? q.right_choice_id ?? null;
        if (!correct && qCorrectId != null) {
          correct = chs.find(c => Number(c.id) === Number(qCorrectId));
        }
        if (!correct) { stat.noCorrect++; continue; }

        const zh = pickZH(correct);
        if (!zh) { stat.noZh++; continue; }
        ...
        if (isVocabPaper(exam.paper)) {
          ...
          stat.skippedPaper++;
          continue; // listening / 其他：暫不處理
        }
      }

      db.execSync?.('COMMIT;');
      console.log(`[syncFromSupabaseToDB] ok words=${stat.w} sentences=${stat.s} | links=${stat.links} skipped={paper:${stat.skippedPaper}, noExam:${stat.noExam}, noQ:${stat.noQ}, noJP:${stat.noJP}, noChoices:${stat.noChoices}, noCorrect:${stat.noCorrect}, noZh:${stat.noZh}}`);
