...
export type PoolChoice = {
  position: number;
  content: string;
  is_correct: boolean;          // 轉成 boolean
  explanation: string;          // 以 string 儲存，null → ''
};
...
/** 只用 Supabase 同步入本地 DB（若已同步過，remote.ts 內會是 no-op/fresh overwrite） */
...
export type PracticeFilter = {
  ...
  month?: '07' | '12' | 'random' | undefined;          // 建議用呢個
  // 兼容舊參數（若傳入會自動映射成 month）
...
export function getAllForFilter(f: PracticeFilter): { pool: PoolQuestion[] } {
  ...
  for (const q of qs) {
    const cs =
      db.getAllSync?.<{ position: number; content: string; is_correct: number | boolean; explanation: string | null }>(
        `
...
export function getQuestionDetail(
  ...
  stem: string;
  passage: string | null;
  options: Array<{ position: number; content: string; is_correct: boolean; explanation: string }>;
  correct?: { position: number; content: string; explanation: string };
} {
  const db = getDB();
  ...
  return {
    ...
    passage: q?.passage ?? null,
    options,
    correct: correct ? { position: correct.position, content: correct.content, explanation: correct.explanation } : undefined,
  };
}
