...
function makeStyles(isDark: boolean) {
  const C = {
    ...
    pending: '#94a3b8', // 淺灰：未做（較易見）
    accent: isDark ? '#93c5fd' : '#2563eb',
    ringWhite: '#ffffff',
    ringGray: isDark ? '#2b2b2b' : '#e5e7eb',
  };
  ...
  const s = StyleSheet.create({
    ...
    pill: {
      height: 72,
      borderRadius: 36,
      borderWidth: 0,           // 外層唔畫邊，改由內層 ring 畫
    ...
    ring: {
      ...
      justifyContent: 'center',
      borderWidth: 2,          // 框線
      backgroundColor: isDark ? '#111' : '#fff', // 解鎖時的白底（亮／暗模式都清晰）
    },
  });
...
function WeekCard({
  ...
  const Pill = (i: number) => {
    ...
    // ring 顏色：✅ 解鎖 = 白框；❌ 未解鎖 = 灰框
    const ringStyle = unlocked
      ? { borderColor: C.ringWhite, backgroundColor: C.card }
      : { borderColor: C.ringGray,  backgroundColor: C.lockedCard };
...
export default function HomeScreen() {
  ...
  return (
    ...
    >
      ...
      {/* 週卡（未解鎖灰、不可按；已解鎖可按日 icon；icon 自帶圓形框） */}
