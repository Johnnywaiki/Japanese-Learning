...
function makeStyles(isDark: boolean) {
  ...
  const s = StyleSheet.create({
    ...
    // 可做狀態的文字樣式（放大版）
    pillText: { fontSize: 20, fontWeight: '900', color: C.white },
  });
...
type WeekCardProps = {
  ...
  onLayoutWeek?: (week: number, y: number) => void; // ★ 新增：回報每張週卡的 y 位置
  s: ReturnType<typeof makeStyles>['s'];
  C: ReturnType<typeof makeStyles>['C'];
};

function WeekCard({
  level, category, week, unlocked, completedSet, onPickDay, onLayoutWeek, s, C,
}: WeekCardProps) {
  ...
  const Pill = (i: number) => {
    ...
    const content = isCan
      ? <RNText style={s.pillText}>開始練習</RNText>
      : <IconRender spec={ICONS[i]} color={isDone ? C.white : C.lockedText} />;
    ...
    return (
      <RNPressable
        ...
        accessibilityLabel={isCan ? '開始練習' : isDone ? '已完成' : '已鎖定'}
      >
        {content}
      </RNPressable>
    );
  ...
  return (
    <RNView onLayout={onLayout} style={[s.card, !unlocked && s.cardLocked]}>
...
export default function HomeScreen() {
  ...
  // ★ 新增：ScrollView ref + 週卡 y 位置記錄
  const scrollRef = useRef<RNScrollView | null>(null);
  const weekYRef = useRef<Record<number, number>>({}); // week -> y

  const onLayoutWeek = useCallback((week: number, y: number) => {
    weekYRef.current[week] = y;
  }, []);

  const scrollToWeek = useCallback((week: number) => {
    const y = weekYRef.current[week] ?? 0;
    // 稍微向上留白一點（12px）
    ...
    // 某些機型第一次還未量完，再補一次
    setTimeout(() => {
      const y2 = weekYRef.current[week] ?? target;
      scrollRef.current?.scrollTo({ y: Math.max(0, y2 - 12), animated: true });
    }, 160);
  }, []);

  // 初始：讀 level、pick、解鎖週、完成記錄，並自動捲到最新解鎖
  useEffect(() => {
    (async () => {
  ...
  // 回到 Home 時，重新撈最新進度（顏色/解鎖即時更新）並捲到最新解鎖週
  useFocusEffect(
    useCallback(() => {
      let alive = true;
      (async () => {
  ...
  const selectPick = useCallback(async (cat: Category) => {
    ...
    if (!level) return;
    const u = await getUnlockedWeek(level, cat);
    setUnlocked(u);
    // 換類別亦自動捲
    scrollToWeek(u);
  }, [level, scrollToWeek]);
    ...
    >
      ...
      {Array.from({ length: 10 }, (_, i) => i + 1).map((week) => (
        <WeekCard
          ...
          onPickDay={onPickDay}
          onLayoutWeek={onLayoutWeek}   // ★ 回報 y 位置
          s={s}
          C={C}
        />
      ))}
