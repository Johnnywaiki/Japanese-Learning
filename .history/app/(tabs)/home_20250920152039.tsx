    ...
    >
      <RNView style={s.cardHeader}>
        ...
        <RNView style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          <RNPressable
            onPress={() => !locked && onStart(week)}
            disabled={locked}
            style={({ pressed }) => [
              s.startBtn,
              locked && s.startBtnDisabled,
              !locked && pressed && { opacity: 0.85 },
            ]}
          >
            <RNText style={[s.startBtnText, locked && s.textLocked]}>開始</RNText>
          </RNPressable>
        </RNView>
      </RNView>
...
export default function HomeScreen() {
  ...
  const startSession = useCallback(
    async (week: number) => {
      ...
      // 目前用 kind: 'language'（含 vocab+grammar）
      await useQuiz.getState().init({
        level: level as any, // N1..N5
        kind: 'language',
        // extra: { week }, // 如果之後要把週傳去 practice
      } as any);
  ...
  return (
    <RNScrollView
      ...
      scrollIndicatorInsets={{ top: 8, bottom: 16 }}
      nestedScrollEnabled
      scrollEnabled
    >
      {/* 頂部：顯示程度 / 解鎖進度 */}
