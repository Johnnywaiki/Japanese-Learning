...
/** 判斷某日（1..7）在該週的狀態：'done' | 'pending' | 'locked'
 * 規則：
 * - day1、day2 初始可做（橙）
 * - day≥3 只有在 day1..(day-1) 全部完成時才可做
 * - 已完成永遠為 done（綠）
 */
function getDayState(args: {
  level: Level;
  ...
  const { level, category, weekUnlocked, week, day, completedSet } = args;
  if (!weekUnlocked || !category) return 'locked';

  const token = tokenOf(level, category, week, day);
  if (completedSet.has(token)) return 'done';

  if (day <= 2) return 'pending';
...
function WeekCard({
  level, category, week, weekUnlocked, completedSet, onPickDay, s, C,
}: WeekCardProps) {
  ...
  const Pill = (i: number) => {
    ...
    const bgStyle =
      state === 'done' ? { backgroundColor: C.done, borderColor: C.done }
      : state === 'pending' ? { backgroundColor: C.pending, borderColor: C.pending }
      : { };

    const baseStyle = [s.pill, !weekUnlocked && s.pillLocked, bgStyle];
...
export default function HomeScreen() {
  ...
  const [unlocked, setUnlocked] = useState<number>(1);          // 已解鎖嘅「週」最大值
  ...
  const onPickDay = useCallback(
    async (week: number, dayIndex: number) => {
      ...
      // 週是否已解鎖？
      const weekUnlocked = week <= unlocked;

      // 依規則檢查：done / pending / locked
      const state = getDayState({
        level,
        category: pick,
        weekUnlocked,
      ...
      if (state === 'locked') {
        Alert.alert('未可挑戰', '請先完成前面未完成嘅題目，再解鎖到下一日。');
        return;
      }
