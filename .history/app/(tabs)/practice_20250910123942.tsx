...
export default function PracticePlayScreen() {
  ...
  const onSelect = (opt: QuizItem) => {
    if (lastCorrect !== undefined) return; // 已提交，唔俾再改
    setPicked(opt);
    setWarn('');
  };
    ...
    <View style={s.wrap}>
      ...
      {lastCorrect !== undefined && (
        <View
          style={[
            s.result,
            { backgroundColor: lastCorrect ? '#e8f7ee' : '#fde8ea', borderColor: lastCorrect ? '#b8e6c8' : '#fccbd2' },
          ]}
        >
          <Ionicons
      ...
      {lastCorrect !== undefined ? (
        ...
        >
          <Text
            style={[s.bottomBtnText, !picked && s.bottomBtnTextDisabled]}
          >
            提交
          </Text>
        </Pressable>
