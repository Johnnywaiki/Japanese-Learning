...
function computeDayStatuses(params: {
  ...
  // day1, day2 初始可做（如果未做）
  res[0] = isDone(1) ? 'done' : 'available';
  res[1] = isDone(2) ? 'done' : 'available';

  // day3..day7：必須前面全部 done 先可做
  for (let d = 3; d <= 7; d++) {
    if (isDone(d)) { res[d - 1] = 'done'; continue; }
    const allPrevDone = Array.from({ length: d - 1 }, (_, i) => i + 1).every(isDone);
    res[d - 1] = allPrevDone ? 'available' : 'locked';
  }
  return res;
}
...
function WeekCard({
  ...
  const renderPill = (i: number) => {
    ...
    if (!weekUnlocked) return inner; // 整週鎖住 → 唔俾按
...
export default function HomeScreen() {
  ...
  // 🔔 監聽 bus（每日完成時 practice-daily 會 bump()）
  ...
  // 禁止跳級：如果 status 係 locked → 提示；available/done → 進入當日練習
