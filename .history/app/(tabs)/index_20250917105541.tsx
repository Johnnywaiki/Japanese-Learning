...
export default function PracticeFilterScreen() {
  ...
  const [level, setLevel] = useState<Topic | 'N2-N3-random' | 'all'>(
    'N2-N3-random',
  );
  const [year, setYear] = useState<number | 'random' | null>('random');
  const [session, setSession] = useState<'July' | 'December' | 'random'>(
    'random',
  );
      ...
      <ScrollView contentContainerStyle={s.wrap}>
        ...
        <View style={s.row}>
          {[
            ...
            >
              <Text
                style={[s.pillText, level === (x.k as any) && s.pillTextActive]}
              >
                {x.label}
              </Text>
            </Pressable>
        ...
        <View style={s.row}>
          {(['N1', 'N2', 'N3', 'N4', 'N5'] as const).map((k) => (
            ...
            >
              <Text style={[s.pillText, level === k && s.pillTextActive]}>
                {k}
              </Text>
            </Pressable>
          ))}
        ...
        <View style={s.row}>
          ...
          >
            <Text style={[s.pillText, year === 'random' && s.pillTextActive]}>
              隨機
            </Text>
          </Pressable>
          ...
          >
            <Text style={[s.pillText, year === null && s.pillTextActive]}>
              未選
            </Text>
          </Pressable>
        </View>
          ...
          <View style={[s.box, { marginTop: 6 }]}>
            <View style={s.rowWrap}>
              {YEARS_11_20.map((y) => (
                ...
                >
                  <Text style={[s.pillTextSm, year === y && s.pillTextActive]}>
                    {y}
                  </Text>
                </Pressable>
              ))}
        ...
        <View style={s.row}>
          {[
            ...
            >
              <Text
                style={[s.pillText, session === (x.k as any) && s.pillTextActive]}
              >
                {x.label}
              </Text>
            </Pressable>
        ...
        <View style={s.row}>
          {[
            ...
            >
              <Text style={[s.pillText, kind === x.k && s.pillTextActive]}>
                {x.label}
              </Text>
            </Pressable>
          ))}
