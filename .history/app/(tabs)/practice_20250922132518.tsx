...
export default function PracticeScreen() {
  ...
  const {
    ...
    mode,   // 'daily' | 'mock'
    meta,   // daily: DailyMeta; mock: PracticeFilter
  } = useQuiz();
  ...
  const submitAndMaybeFinish = useCallback(async () => {
    ...
    try {
      if (mode === 'daily' && meta && 'level' in meta && 'category' in meta && 'week' in meta && 'day' in meta) {
      ...
      if (mode === 'daily') router.replace('/(tabs)/home');
  ...
  const onPickNewPaper = useCallback(() => { router.replace('/(tabs)'); }, []);
