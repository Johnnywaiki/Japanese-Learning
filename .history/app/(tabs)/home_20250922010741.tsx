...
// ---------- helper：由 week/day 組 daily_key ----------
function dailyKeyOf(level: Level, cat: Category, week: number, day: number) {
  // 1..10 週，每週 7 日 → 0001..0070
  const idx = (week - 1) * 7 + day; // 1-based
  const catUpper = (cat === 'grammar' ? 'GRAMMAR' : 'VOCAB') as 'GRAMMAR' | 'VOCAB';
...
function WeekCard({
  ...
  const Pill = (i: number) => {
    ...
    if (!unlocked) {
      return (
        <RNView key={ICONS[i].key} style={baseStyle} testID={`pill-${ICONS[i].key}`} accessible accessibilityLabel={`${ICONS[i].key}-locked`}>
...
export default function HomeScreen() {
  ...
  // ✅ 按某粒日 → 計 daily_key → 用 daily 模式入練習
