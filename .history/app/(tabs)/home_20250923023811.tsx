...
function makeStyles(isDark: boolean) {
  ...
  const s = StyleSheet.create({
    ...
    offset: { marginTop: 36 },
    pill: {
      height: 72, borderRadius: 36, borderWidth: 2, borderColor: C.border, backgroundColor: C.pillBg,
    ...
    pillCanDo: { backgroundColor: C.canDo, borderColor: C.canDo },
    // icon 包一層方便可選加陰影
...
// 計算該日狀態
type DayState = 'done' | 'canDo' | 'locked';
function calcDayState(params: {
  level: Level; category: Category; week: number; day: number;
  unlockedWeek: number; completed: Set<string>;
}): DayState {
  const { level, category, week, day, unlockedWeek, completed } = params;

  // 整週未解鎖 → 全部鎖
  if (week > unlockedWeek) return 'locked';

  const token = tokenOf(level, category, week, day);
  if (completed.has(token)) return 'done';

  // day1 / day2：一開始已可做
  if (day <= 2) return 'canDo';

  // day >= 3：必須前面所有日已完成先可做
  const allPrevDone = Array.from({ length: day - 1 }, (_, i) =>
    tokenOf(level, category, week, i + 1)
  ).every(tk => completed.has(tk));
  return allPrevDone ? 'canDo' : 'locked';
}
...
function WeekCard({
  ...
  const Pill = (i: number) => {
    ...
    const state: DayState = category
      ? calcDayState({ level, category, week, day, unlockedWeek: unlocked ? week : 0, completed: completedSet })
      : 'locked';

    const base = [s.pill];
    let iconColor = C.lockedText;

    if (state === 'done') {
      base.push(s.pillDone);
      iconColor = C.white;
    } else if (state === 'canDo') {
      base.push(s.pillCanDo);
      iconColor = C.white;
    } else {
      base.push(s.pillLocked);
      iconColor = C.lockedText;
    }

    const content = (
      <RNView key={ICONS[i].key} style={base}>
        <RNView style={s.iconWrap}>
...
export default function HomeScreen() {
  ...
  // 回頁即 reload（完成 → 變色／解鎖）— 加半拍避免 race
  useFocusEffect(
    useCallback(() => {
      let alive = true;
      (async () => {
        try {
          await new Promise(r => setTimeout(r, 30)); // ⏱️ 讓 AsyncStorage flush 完
          const lv = (await AsyncStorage.getItem(LEVEL_KEY)) as Level | null;
          const pk = (await AsyncStorage.getItem(PICK_KEY)) as Category | null;
          if (!lv || !pk) return;
