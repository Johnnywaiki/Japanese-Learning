...
function makeStyles(isDark: boolean) {
  ...
  const s = StyleSheet.create({
    ...
    next: { padding: 14, borderRadius: 12, backgroundColor: C.activeBg, opacity: 1 },
...
export default function PracticeFilterScreen() {
  ...
  const [goPractice, setGoPractice] = useState(false);
...
const onStartPress = () => {
  const filters: PracticeFilter = {
    level,
    kind,
    ...(typeof year === 'number' ? { year } : {}),           // 年份唔係 number 就唔傳
    ...(session !== 'random' ? { session } : {}),            // 只傳 July/December
  };
  void init(filters).then(() => router.navigate('/(tabs)/practice'));
};

  // 等 Root navigator mount 完先跳頁，避免 navigate 太早
  useEffect(() => {
    if (goPractice && navState?.key) {
      setGoPractice(false);
      setBusy(false);
      router.navigate('/(tabs)/practice');
    }
  }, [goPractice, navState?.key]);
      ...
      <ScrollView contentContainerStyle={s.wrap}>
        ...
        <View style={s.row}>
          {[
            ...
            >
              <Text style={[s.pillText, level === (x.k as any) && s.pillTextActive]}>
                {x.label}
              </Text>
            </Pressable>
          ))}
        ...
        <View style={s.row}>
          <Pressable
            onPress={() => setYear('random')}
            style={[s.pill, year === 'random' && s.pillActive]}
          >
          ...
          <Pressable
            ...
            style={[s.pill, year === null && s.pillActive]}
          >
        ...
        <View style={s.row}>
          {[
            ...
            >
              <Text style={[s.pillText, session === (x.k as any) && s.pillTextActive]}>
                {x.label}
              </Text>
            </Pressable>
          ))}
        ...
        {/* 開始練習 */}
        <Pressable
          style={[s.next, { marginTop: 10 }, busy && s.nextDisabled]}
          onPress={onStartPress}
          disabled={busy}
        >
          <Text style={s.nextText}>{busy ? '載入中…' : '開始練習'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
